...
resource "authentik_provider_oauth2" "providers" {
  ...
  name               = each.key
  client_id          = lookup(jsondecode(var.oauth2_settings), format("%s_client_id", each.key))
  client_secret      = lookup(jsondecode(var.oauth2_settings), format("%s_client_secret", each.key))
  authorization_flow = data.authentik_flow.default-authorization.id
  signing_key        = data.authentik_certificate_key_pair.generated.id
  property_mappings  = concat(data.authentik_scope_mapping.scopes.ids, lookup(each.value, "extra_scopes", []))
  redirect_uris      = [lookup(jsondecode(var.oauth2_settings), format("%s_redirect_url", each.key))]
