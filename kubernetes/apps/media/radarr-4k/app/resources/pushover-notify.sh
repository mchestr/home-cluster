...
if [[ "${radarr_eventtype:-}" == "Test" ]]; then
    ...
    PUSHOVER_MESSAGE="Howdy this is a test notification from ${radarr_instancename:-Radarr-4K}"
fi
...
if [[ "${radarr_eventtype:-}" == "Download" ]]; then
    ...
    printf -v PUSHOVER_TITLE "Movie %s (4K)" "${pushover_title}"
    ...
    printf -v PUSHOVER_URL_TITLE "View movie in %s" "${radarr_instancename:-Radarr-4K}"
fi
...
if [[ "${radarr_eventtype:-}" == "ManualInteractionRequired" ]]; then
    PUSHOVER_PRIORITY="1"
    printf -v PUSHOVER_TITLE "Movie requires manual interaction (4K)"
    printf -v PUSHOVER_MESSAGE "<b>%s (%s)</b><small>\n<b>Client:</b> %s</small>" \
        "${radarr_movie_title}" \
        "${radarr_movie_year}" \
        "${radarr_download_client}"
    printf -v PUSHOVER_URL "%s/activity/queue" "${radarr_applicationurl:-localhost}"
    printf -v PUSHOVER_URL_TITLE "View queue in %s" "${radarr_instancename:-Radarr-4K}"
fi
